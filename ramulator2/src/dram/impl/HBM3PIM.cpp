...
class HBM3PIM : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, HBM3PIM, "HBM3PIM", "HBM3 Device Model with PIM support")
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      //   name       rate   nBL  nCL  nRCDRD  nRCDWR  nRP  nRAS  nRC  nWR  nRTPS  nRTPL  nCWL  nCCDS  nCCDL  nRRDS  nRRDL  nWTRS  nWTRL  nRTW  nFAW  nRFC  nRFCSB  nREFI  nREFISB  nRREFD  nQUANT  nOUTLIER nBL_COMP
      {"HBM3_2Gbps",  {2000,   4,   7,    7,      7,     7,   17,  19,   8,    2,     3,    2,    1,      2,     2,     3,     3,     4,    3,    15,   -1,   160,   3900,     -1,      8,   10,    15,    1}},
    };
    ...
    inline static constexpr ImplDef m_commands = {
      "ACT", "PRE", "PREA", "RD", "WR", "RDA", "WRA", "REFab", "REFsb", "RFMab", "RFMsb", "PIM_QUANTIZE", "PIM_OUTLIER", "RD_COMP", "WR_COMP"
    };
    ...
    inline static const ImplLUT m_command_scopes = LUT (
      m_commands, m_levels, {
        ...
        {"RFMab", "channel"}, {"REFsb", "bank"}, {"PIM_QUANTIZE", "bank"}, {"PIM_OUTLIER", "bank"},
        {"RD_COMP", "column"}, {"WR_COMP", "column"}
      }
    );
    ...
    inline static const ImplLUT m_command_meta = LUT<DRAMCommandMeta> (
      m_commands, {
        ...
        {"PIM_QUANTIZE", {false, false, false, false}}, {"PIM_OUTLIER", {false, false, false, false}},
        {"RD_COMP", {false, false, true, false}}, {"WR_COMP", {false, false, true, false}}
      }
    );

    inline static constexpr ImplDef m_requests = {
      "read", "write", "kvcache_write", "kvcache_read", "all-bank-refresh", "per-bank-refresh", "all-bank-rfm", "per-bank-rfm", "read_comp", "write_comp", "pim_quantize", "pim_outlier"
    };

    inline static const ImplLUT m_request_translations = LUT (
      m_requests, m_commands, {
        {"read", "RD"}, {"write", "WR"}, {"kvcache_write", "WR"}, {"kvcache_read", "RD"},
        {"all-bank-refresh", "REFab"}, {"per-bank-refresh", "REFsb"}, {"all-bank-rfm", "RFMab"}, {"per-bank-rfm", "RFMsb"}, 
        {"read_comp", "RD_COMP"}, {"write_comp", "WR_COMP"}, {"pim_quantize", "PIM_QUANTIZE"}, {"pim_outlier", "PIM_OUTLIER"}
      }
    );

    inline static constexpr ImplDef m_timings = {
      "rate", "nBL", "nCL", "nRCDRD", "nRCDWR", "nRP", "nRAS", "nRC", "nWR", "nRTPS", "nRTPL", "nCWL",
      "nCCDS", "nCCDL", "nRRDS", "nRRDL", "nWTRS", "nWTRL", "nRTW", "nFAW", "nRFC", "nRFCSB", "nREFI", "nREFISB", "nRREFD", "nQUANT", "nOUTLIER", "nBL_COMP"
    };
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      populate_timingcons(this, {
          ...
          // PIM quantization timing (normal path: INT4 compression)
          {.level = "bank", .preceding = {"ACT"}, .following = {"PIM_QUANTIZE"}, .latency = V("nRCDRD")},
          {.level = "bank", .preceding = {"PIM_QUANTIZE"}, .following = {"WR"}, .latency = V("nQUANT")},
          // PIM outlier timing (high-precision path for 1% outliers)
          {.level = "bank", .preceding = {"ACT"}, .following = {"PIM_OUTLIER"}, .latency = V("nRCDRD")},
          {.level = "bank", .preceding = {"PIM_OUTLIER"}, .following = {"WR"}, .latency = V("nOUTLIER")},

          // Compressed commands timing
          {.level = "bank", .preceding = {"ACT"}, .following = {"RD_COMP"}, .latency = V("nRCDRD")},
          {.level = "bank", .preceding = {"ACT"}, .following = {"WR_COMP"}, .latency = V("nRCDWR")},
          {.level = "bank", .preceding = {"RD_COMP"}, .following = {"PRE"}, .latency = V("nRTPS")},
          {.level = "bank", .preceding = {"WR_COMP"}, .following = {"PRE"}, .latency = V("nCWL") + V("nBL_COMP") + V("nWR")},

          // Sub-burst timing for compressed commands (pipelining)
          {.level = "pseudochannel", .preceding = {"RD_COMP"}, .following = {"RD_COMP"}, .latency = V("nCCDS")}, // Simplified
          {.level = "pseudochannel", .preceding = {"WR_COMP"}, .following = {"WR_COMP"}, .latency = V("nCCDS")},
        }
      );
      #undef V
    };
    ...
    void set_actions() {
      ...
      m_actions[m_levels["channel"]][m_commands["PREA"]] = Lambdas::Action::Channel::PREab<HBM3PIM>;
    ...
    void set_preqs() {
      ...
      m_preqs[m_levels["bank"]][m_commands["PIM_QUANTIZE"]] = Lambdas::Preq::Bank::RequireRowOpen<HBM3PIM>;
      m_preqs[m_levels["bank"]][m_commands["PIM_OUTLIER"]] = Lambdas::Preq::Bank::RequireRowOpen<HBM3PIM>;
    };
    ...
    void set_rowhits() {
      ...
      m_rowhits[m_levels["bank"]][m_commands["PIM_QUANTIZE"]] = Lambdas::RowHit::Bank::RDWR<HBM3PIM>;
      m_rowhits[m_levels["bank"]][m_commands["PIM_OUTLIER"]] = Lambdas::RowHit::Bank::RDWR<HBM3PIM>;
      m_rowhits[m_levels["bank"]][m_commands["RD_COMP"]] = Lambdas::RowHit::Bank::RDWR<HBM3PIM>;
      m_rowhits[m_levels["bank"]][m_commands["WR_COMP"]] = Lambdas::RowHit::Bank::RDWR<HBM3PIM>;
    };
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["PIM_QUANTIZE"]] = Lambdas::RowOpen::Bank::RDWR<HBM3PIM>;
      m_rowopens[m_levels["bank"]][m_commands["PIM_OUTLIER"]] = Lambdas::RowOpen::Bank::RDWR<HBM3PIM>;
      m_rowopens[m_levels["bank"]][m_commands["RD_COMP"]] = Lambdas::RowOpen::Bank::RDWR<HBM3PIM>;
      m_rowopens[m_levels["bank"]][m_commands["WR_COMP"]] = Lambdas::RowOpen::Bank::RDWR<HBM3PIM>;
    };
